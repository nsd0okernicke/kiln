...
see `roles/human-in-the-loop.md` → "Receiving Messages". You never call `wait_for_message()`,
run `/kiln-receive`, or poll for anything yourself in this profile. Read what the inbox pane
...
3. **Send handoff** — call `python .kiln/tools/set-status.py {{ROLE}} handoff --mode={{MODE}}` first. Then run `/kiln-handoff` (or `kiln send`, per `roles/human-in-the-loop.md`). Do not
   consider the turn over until a queued row is confirmed.
