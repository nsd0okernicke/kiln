...
4. **Delegate the work** — do not implement anything yourself. Delegate this task entirely to the
   custom agent named `{{ROLE}}-worker` (defined in `.codex/agents/{{ROLE}}-worker.toml`) using your
   multi-agent spawn tools. Give it the full content of the received message, your current
   branch/worktree, and an explicit request for a final report of what was implemented/verified and
   which files were touched. The `{{ROLE}}-worker` agent already has your role's work process,
   quality gates, and standards baked into its own definition — do not repeat them in the task, and
