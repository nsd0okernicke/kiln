...
- **Acceptance tests** (integration tests) verify complete user journeys against real infrastructure (databases, message queues). Write them in `infrastructure/<layer>/tests/` (e.g., `infrastructure/api/tests/`, `infrastructure/db/tests/`). Use descriptive test names: `test_user_can_search_books_by_title_and_author()`.
- Use pytest with `@pytest.mark.integration` for acceptance/integration tests to separate them from unit tests.
- Acceptance tests must run against **real services via Testcontainers**: PostgreSQL in containers, RabbitMQ in containers. Do not mock these boundaries.
- Run unit tests (domain/, application/) with `pytest -m 'not integration'`. Run all tests (including acceptance) with `pytest` (default).
...
- Run the relevant local verification command before handoff whenever the project has one: `pytest` (all tests) or `pytest -m 'not integration'` (fast unit tests only, depending on handoff context).
