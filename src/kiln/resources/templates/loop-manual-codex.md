...
`kiln-db` directly — Codex has no confirmed support for a long-blocking MCP tool call, so
unlike Claude's `/kiln-receive` (which blocks on `kiln-channel`'s `wait_for_message()`), this
loop polls the same way Copilot's does. Do not call `wait_for_message()` — that's a
`kiln-channel` tool and this role's MCP config only has `kiln-db`.
...
1. **Receive** — call `python .kiln/tools/set-status.py {{ROLE}} waiting --mode={{MODE}}` first. Then call `query`:
   ...
   entry to `logbook.md` (timestamp, full message content) — if auto-compact fires later in
   the cycle and this tool result is gone from context, re-read `tmp/handoff-in.md` to
