...
`kiln-db` directly — Copilot has no confirmed support for a long-blocking MCP tool call, so
unlike Claude's `/kiln-receive` (which blocks on `kiln-channel`'s `wait_for_message()`), this
loop polls instead. Do not call `wait_for_message()` — that's a `kiln-channel` tool and this
role's MCP config only has `kiln-db`.
