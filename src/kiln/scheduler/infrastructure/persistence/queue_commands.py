"""SQLite mutation commands for the scheduler message queue."""
...
def mark_processing(db_path: str | Path, message_id: str) -> bool:
    ...
    return _set_status(db_path, message_id, STATUS_PROCESSING, stamp_column="started_at")
...
def mark_processed(db_path: str | Path, message_id: str) -> bool:
    ...
    return _set_status(
        ...
        message_id,
        STATUS_PROCESSED,
        stamp_column="processed_at",
        extra_stamp_column="finished_at",
    )
...
def mark_failed(db_path: str | Path, message_id: str, error: str) -> bool:
    ...
    with closing(connect(db_path)) as conn:
        ...
        if not can_transition(current, MessageStatus.FAILED):
            ...
            return False
        cur.execute(
            f"UPDATE messages SET status=?, error=?, finished_at={_UTC_NOW} WHERE id=?",
...
def name_work_item(db_path: str | Path, message_id: str, work_item: str) -> bool:
    """Attach the name created by the first role to its formerly unnamed inbound row."""
    with closing(connect(db_path)) as conn:
        cur = conn.execute(
            "UPDATE messages SET work_item=? WHERE id=? AND work_item IS NULL",
...
def insert_handoff(
    ...
    `created_at` is generated by SQLite in UTC.
