    ...
    switch ($Agent) {
        ...
        "copilot" {
            ...
            if ($FrameworkRoot -and $DatabasePath) {
                ...
                $command = "copilot mcp remove kiln-db -ErrorAction SilentlyContinue; copilot mcp add kiln-db -- python `"$serverPath`" `"$DatabasePath`"; copilot --allow-all $debugFlag".Trim()
            } else {
...
function Get-WindowsTerminalAgentCommand {
    ...
    switch ($Agent) {
        ...
        "copilot" {
            ...
            if ($FrameworkRoot -and $DatabasePath) {
                $serverPath = "$FrameworkRoot\kiln\mcp-server\kiln_db_server.py"
                $baseCmd = "copilot mcp remove kiln-db -ErrorAction SilentlyContinue; copilot mcp add kiln-db -- python `"$serverPath`" `"$DatabasePath`"; copilot --allow-all $copilotDebugFlag"
            } else {
