...
function Write-GeneratedCLAUDEmd {
    ...
    $content += "# ⚠️ STARTUP REQUIRED: Subscribe to Your Inbox Now`n`n"
    $content += "Before doing ANY other work, you MUST complete these startup steps:`n`n"
    $content += "## Step 1: Subscribe to Inbox Resource`n"
    $content += "Use the kiln-db MCP resource subscription feature. You need to subscribe to: **kiln://inbox/$Role**`n`n"
    $content += "This enables push notifications. When another agent sends you a message, you will immediately receive a `notifications/resources/updated` event and can react in real-time.`n`n"
    $content += "## Step 2: Confirm Subscription`n"
    $content += "After subscribing, write a message confirming: **'I have subscribed to kiln://inbox/$Role and am ready to receive messages.'**`n`n"
    $content += "This confirms to the system that you are listening for incoming messages.`n`n"
    $content += "## Step 3: Monitor for Notifications`n"
    $content += "After subscribing, the MCP server will notify you (via `notifications/resources/updated`) whenever a new message arrives in your inbox. When this happens, proceed to Step 4.`n`n"
    $content += "## Step 4: Read Inbox & Process Messages`n"
    $content += "When you receive a notification (or whenever you need to check): Call `read_inbox(role=""$Role"", branch=""$CurrentBranch"")`  to fetch messages. Then:`n"
    $content += "- Process the message according to your role`n"
    $content += "- Call `mark_delivered(message_id=""<id>"")` after reading`n`n"
    $content += "---`n`n"
