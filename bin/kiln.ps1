...
function Write-GeneratedCLAUDEmd {
    ...
    $content += "# ⚠️ Startup Required: Subscribe to Your Inbox`n`n"
    $content += "Before processing any messages, you MUST call this tool once at session startup:`n`n"
    $content += "```\nsubscribe_to_inbox()\n```\n`n"
    $content += "This enables push notifications. After subscribing, when other agents send you messages, you'll immediately receive a `notifications/resources/updated` event.`n`n"
    $content += "Then, to process incoming messages: call `read_inbox(role=""$Role"")` when you receive a notification, then `mark_delivered(message_id=""<id>"")` after reading.`n`n"
    $content += "---`n`n"
