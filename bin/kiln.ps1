...
function Write-GeneratedCLAUDEmd {
    ...
    $content += "# Kiln Agent Communication`n`n"
    $content += "You are launched with `--channels server:kiln-db`, which means you automatically receive handoff messages from other agents as `<channel>` events.`n`n"
    $content += "**When you see a `<channel>` event:**`n"
    $content += "- Read the message content`n"
    $content += "- Call `read_inbox(role=""$Role"")` to fetch the queued messages`n"
    $content += "- Process messages where the meta.target matches your role`n"
    $content += "- Call `mark_delivered(message_id=""<id>"")` after reading each message`n"
    $content += "- Call `mark_processed(message_id=""<id>"")` after completing the work`n`n"
    $content += "**To send a handoff to another agent:**`n"
    $content += "- Call `send_message(sender=""$Role"", target=""<target-role>"", content=""<message>"")`n"
    $content += "- The MCP server will automatically push a `<channel>` event to the target agent`n`n"
    $content += "---`n`n"
...
function Get-WindowsTerminalAgentCommand {
    ...
    switch ($Agent) {
        "claude" {
            if ($DisplayName) {
                return "claude --model $Model --permission-mode bypassPermissions --channels server:kiln-db --mcp-config ./.mcp.json -n ""$DisplayName"" $debugFlag".Trim()
            } else {
                return "claude --model $Model --permission-mode bypassPermissions --channels server:kiln-db --mcp-config ./.mcp.json $debugFlag".Trim()
            }
        }
        "copilot" {
            # Register MCP server explicitly for Copilot CLI
