...
function Write-GeneratedCLAUDEmd {
    ...
    $content += "## Step 2: Monitor for Notifications`n"
    $content += "After subscribing, the MCP server will notify you (via `notifications/resources/updated`) whenever a new message arrives in your inbox. When this happens, proceed to Step 3.`n`n"
    $content += "## Step 3: Read Inbox & Process Messages`n"
