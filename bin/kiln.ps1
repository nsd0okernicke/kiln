...
function Write-GeneratedCLAUDEmd {
    ...
    $content += "# Kiln MCP Setup`n`n"
    $content += "Your MCP server (kiln-db) automatically subscribes you to **kiln://inbox/$Role** on first contact. When other agents send you messages, you'll receive push notifications (`notifications/resources/updated`) immediately.`n`n"
    $content += "To process incoming messages: call `read_inbox(role=""$Role"")` when you receive a notification, then `mark_delivered(message_id=""<id>"")` after reading.`n`n"
    $content += "---`n`n"
