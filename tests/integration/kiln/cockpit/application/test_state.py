...
class TestRoleRows:
    ...
    def test_a_wrapper_role_falls_back_to_the_profiles_model(self):
        ...
        sessions = [RoleSession("human-in-the-loop", "claude", "Human", "agent", "claude-sonnet-5")]
...
class TestBuildAttention:
    ...
    def test_a_failed_row_is_retryable_and_a_review_is_not(self):
        ...
        items = cockpit_state.build_attention(
            failed=failed,
            awaiting_human=[_row(target="human-in-the-loop")],
            messages=[],
            now_local=NOW_LOCAL,
            human_role="human-in-the-loop",
        )
...
class TestBuildState:
    def test_the_payload_carries_every_section_the_page_renders(self):
        ...
        payload = cockpit_state.build_state(
            ctx,
            _snapshot(messages=[_row()]),
            work_items=[_row()],
            cycles={"ORDER-INTAKE": 1},
            failed=[],
            awaiting_human=[],
            activity_limit=5,
        )
    ...
    def test_it_offers_the_known_work_items_newest_first(self):
        ...
        payload = cockpit_state.build_state(
            ...
            _snapshot(),
            work_items=[],
            cycles={"CAT-3": 4, "cat-1-search-books": 6},
            failed=[],
            awaiting_human=[],
            activity_limit=5,
        )
