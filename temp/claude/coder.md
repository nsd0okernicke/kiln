...
4. **Work** — apply your role rules. The Role section above defines your specific
   ...
   forward as-is to `refactorer` and skip steps 5–7.
...
6. **Squash** — squash all your commits since the merge commit into one:
   ...
   git commit -m "[Coder] <feature name> - TDD implementation of <what>"
   ```

7. **Send handoff** — call `write_query` to INSERT into `messages` with `target='refactorer'`,
   `branch='run2406'`, and `content` formatted per Handoff Message Format in Workflow Rules.
   Verify: `SELECT id FROM messages WHERE sender='coder' AND branch='run2406' ORDER BY created_at DESC LIMIT 1`
...
- **Role**: `coder`
- **Branch**: `run2406` — this is the ROOT branch. Do NOT substitute your worktree sub-branch (e.g. `run2406-coder` would be wrong).
- **Worktree**: `coder`
